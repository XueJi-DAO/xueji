 ...
 */
export function createWebRTCProvider(id: string, yjsDocMap: Map<string, Y.Doc>): Provider {
  ...
  const provider = new WebrtcProvider(`${id}/${idSuffix++}`, doc, {
    ...
    signaling: window && window.location.hostname === 'localhost' ? ['ws://localhost:1235'] : [],
  })
