...
**重点**: 对话式智能交互 + 知识可视化 + 知识地图 + 记忆宫殿 + 自我管理 GTD => 学习积累 => 协同创作

## 设计理念

极致体验: **智能 简捷 精准 高效 安全**

- **大道至简** 解决问题的方法论是通用的；**以人为本**增强人的能力的工具及知识, 人的个性化需求是无限的
- **基于统一语义**: **语义计算**,化繁为简,融合不同领域学科,人与机器的知识。语义->概念->信息->知识->智能
- **链接知识与人**: 互联数据, 把知识链接成网络,加强人与知识的关联
- **一切皆文档**: 基于语义组件, 插件, 智能模板, **定制**, 按需组合成你需要的应用, 一个地方解决所有问题
- **智能交互**: 对话式交互, 智能推理推荐, 辅助创作, 引导思考, 提问, 自然交互, 学习增强, 与专属助理一同成长
- **知识可视化**: 多视图多层次与动态知识交互(纯文本, 表格, 导图, 图表, 日历, 项目, VR, 5W2H1E)
...
> 覆盖: 收集 整理 理解 应用(检索,问答) 创新(创作) 分享

- 智能文档: 对话生成式 HUI, 多视图多模式多层次实时协作画板, **语义组件**插件智能模板应用, AIGC, DSL
- 专属助理: **学记助理**, 自然语言交互, 能动性, 生成推荐提醒
- 虚拟空间: **记忆宫殿**, 3D 可视化元宇宙, 发现
- 自我管理: 原则目标时间精力数据管理, GTD
- 安全可信: 数据存储, 身份认证授权, 智能合约, 贡献积分
...
- **智能文档**: 应用也是一个文档, 文档即应用
  ...
  - 画板引擎: 知识可视化交互
    ...
    - **多层次**: 语义逻辑时空人层次与知识进行交互, 在不同层级展示不同视图。每个文档都可作为为根文档节点, 不同场景目的层次角度粒度重点
    - **对话式**: 接入智能助理自动推荐补全生成, 引导按需展示，浮现按钮,弹窗
    - **多模式**: 浏览, 编辑, 模板编辑, 切换模板, 另存模板
    - **多视图**: 画板(富文本+画图+神笔+2D3D), 日历地图卡片列表表格图表导图, 自定义混合视图(项目任务看板), 视图保存/锁定/联动/缩放(时空繁简难易,抽象具体)/层级(义层+图层+模板)/详细程度/范畴, 字体主题布局排版。过渡动画。
    - **实时协作**: 操作到任务项目不同层次的协同, 邀请协作, 实时音视频

  - 工作台: 导航区: 目录, 上一下一文档, 快捷方式, 最近; 监控区(通知监控, 辅助视图), 工作区(5 主画板), 属性区(文档知识属性)
    - 工具箱: 画笔, 形状, 排列布局, 垃圾箱, 收集箱
    - 动作: 文本手势语音画图拖放点击
    - 操作: 新建(基于模板, 目的, 输入), 建立修改关系, 折叠, 插入, 附件, 布局;播放移动复制粘贴删除撤销重做锁定恢复
    - 收藏分享导入导出备份版本历史投票点赞评论克隆发布归档购买授权订阅,通知: 接收信息频度时间, 属性继承, 关键词链接高亮。
    - 插件: 弹幕, 标签云, 向导,文字转图形图像动画视频, 画图生成, 看图说话, 文档批处理, 定时任务
    - 检索: 搜索纠错推荐, 组合搜索, 最近搜索, 搜索范围(分类+全文+知识库)
    - 筛选: 不同关系或属性过滤筛选文档; 排序(重要, 紧急), 分组, 标记(基线, 标签,注解,书签, 颜色, 图标), 工作流
    - 安全及隐私: 自动保存同步,备份, 加密权限, 知识产权

- **领域语言**: 面向智能知识管理的人机交互语言, 更高效的人机交互方式。组合插件,调用 API,表示处理文档知识, 命令行, 声明式+命令式, 语法提示, 自动修正命令提示, 传输语法, 可视化语法。
- **专属助理**: 小五/添香/宇轩, 虚拟形象装扮升级,智能对话引导启发提示推荐提醒,辅助整理学习渐进推理决策, 根据用户目的进行提示词补全增强，对话转知识点
- **智能模板**: 应用可以含多个模板或切换模板, 日记+学习笔记+待办+名片,一页纸项目管理, 整理思路, 5W2H1E, PDCA, 金字塔原理, 世界咖啡
- **资源**: 模板组件插件知识应用, 技能包物品徽章
- **内置应用**: 一切皆文档, 核心文档+内置文档+开放文档+用户文档
  - **知识管理**: 知识收集整理迭代,知识问答,概念图谱,知识地图,辅助学习编程,画画,语言
...
- 文档模型: 文档引擎, 语义化, 对话式交互, UI 组装, 内容注入
...
- 语义模型: 语义引擎, 结构: 正十二面体, 层级: 语义->概念->信息->模式-->知识+能力->智能
- 智能模型: 智能引擎，认知智能(人,能动性,知识,计算)
  - 认知层: 理解学习推理，大模型
  - 链接层: 调取记忆辅助推理, 推理学习形成记忆
  - 记忆层: 短期缓存，原则性格知识长期存储
  - 决策层: 情感决策
  - 生成层: 输出语音文字图片
- 世界模型: 虚拟世界,空间,用户
...
语义S-->概念C-->信息I(实体 Entity--关系 Relation--属性 Property--功能 Function--公理 Axiom)-->模式P(结构)-->知识(人场景目的)+能力(能量)-->智能
...
Reactjs,Nextjs,MUI,emotion,UnoCss,Ts,impressjs,markdown-it,slatejs,threejs,twojs,d3js,openlayers,GraphQL,apollo,trpc,apisix
